...
def main():
    user_id = session.get('user_id')
    '''   
        first filter:  current user (gender_interest)     
        display the users who have atleast ome same interests
              
    '''
    print("USER ID: ", user_id) 

    # Current user Gender_Interest
    cursor = mysql.connection.cursor(DictCursor)
    cursor.execute("SELECT gender_interest FROM user_details WHERE user_id = %s", (user_id,))
    user_gender_interest = cursor.fetchone()['gender_interest']
    print("Gender interest: ",user_gender_interest)

    
    #Current user Interets
    cursor = mysql.connection.cursor(DictCursor)
    cursor.execute("SELECT interests FROM flirtit_db.user_details WHERE user_id = %s", (user_id,))
    user_interests = cursor.fetchone()['interests']
    print("User Interests: ",user_interests)
    
    #Filtering - show all - atleast 1 same interest 
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("""
        SELECT id, user_id, full_name, age, interests, gender, gender_interest, profile_pic 
        FROM user_details
        WHERE gender = %s
        AND user_id != %s
    ...
    match_interests = cursor.fetchall()
    print("Match:", match_interests)
    cursor.close()
    print("Im in!!!!!!!!!!")  
##########################################

    return render_template('main.html', match_interests = match_interests)
